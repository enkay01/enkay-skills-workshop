            ...
            "tools/call" => {
                ...
                let content_str = match name {
                    ...
                    "fast_read_batch" => {
                        ...
                        view_batch_files(&paths, &opts, 500 * 1024)
                    }
                    "grep_context" => {
                        let paths: Vec<PathBuf> = arguments.get("paths")
                            .and_then(|v| v.as_array())
                        ...
                        let context_lines = arguments.get("context_lines").and_then(|v| v.as_u64()).unwrap_or(10) as usize;
                        let max_matches = arguments.get("max_matches").and_then(|v| v.as_u64()).unwrap_or(5) as usize;
                        ...
                        match search_context(&paths, pattern, context_lines, max_matches, is_regex, case_sensitive) {
                            Ok(matches) => serde_json::to_string_pretty(&matches).unwrap_or_default(),
                            Err(e) => format!("Error: {}", e),
                        }
                    }
                    "poll_service" => {
                        let pid = arguments.get("pid").and_then(|v| v.as_i64()).map(|v| v as i32);
                        let port = arguments.get("port").and_then(|v| v.as_u64()).map(|v| v as u16);
                    ...
                    "git_snapshot" => {
                        ...
                        match get_working_tree_snapshot(repo_dir) {
                            Ok(snap) => serde_json::to_string_pretty(&snap).unwrap_or_default(),
                            Err(e) => format!("Error: {}", e),
                        }
                    }
